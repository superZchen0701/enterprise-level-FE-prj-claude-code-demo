# 项目结构说明文档

## 项目概览

一个基于 Vite 构建的 Vue 3 移动端 h5 应用，主要包含兑换记录追踪等业务模块。

## 目录结构

```
项目根目录/
├── index.html                    # HTML 入口（Vite 默认从根目录读取）
├── public/                      # 静态资源目录（直接复制到输出目录）
│   └── favicon.ico              # 网站图标
├── src/                         # 源代码目录
│   ├── main.ts                  # 入口文件，全局注册组件
│   ├── App.vue                  # 根组件
│   ├── router/                  # 路由配置
│   │   └── index.ts             # Vue Router 实例及路由规则
│   ├── views/                   # 页面级组件
│   ├── components/              # 公共组件
│   ├── api/                     # API 接口封装
│   ├── utils/                   # 工具函数
│   ├── mock/                    # Mock 数据（开发环境模拟）
│   └── assets/                  # 静态资源（图片、字体等）
├── openspec/                    # OpenSpec 需求管理
│   ├── changes/                 # 变更目录
│   │   └── archive/             # 已归档变更
│   ├── specs/                   # 主规范库
│   ├── config.yaml              # OpenSpec 配置
│   └── README.md                # OpenSpec 使用指南
├── .claude/                     # Claude Code 配置
├── docs/                        # 项目文档
│   └── ai-context/              # AI 上下文文档
├── .env.development             # 开发环境变量
├── .env.production              # 生产环境变量
├── package.json                 # 项目依赖配置
├── vite.config.ts               # Vite 构建配置
├── tsconfig.json                # TypeScript 项目配置（路径别名）
└── CLAUDE.md                    # Claude Code AI 行为规范
```

## 技术栈

| 技术 | 版本 | 用途 |
|------|------|------|
| Vue | 3.5.x | 前端框架（Composition API） |
| TypeScript | 5.5.x | 类型安全 |
| Vue Router | 4.x | 路由管理 |
| Vant | 4.x | 移动端 UI 组件库 |
| Vite | 6.x | 构建工具 |
| ESLint | 8.x | 代码检查 |
| vue-tsc | 2.x | TypeScript 类型检查 |

## 目录职责

### src/main.ts
Vue 应用入口文件，负责：
- 使用 `createApp` 创建 Vue 应用实例
- 全局注册 Vant 组件（NavBar、Cell、Loading、Toast）
- 挂载路由

### src/router/index.ts
路由配置，负责：
- 使用 `createRouter` + `createWebHistory` 创建路由
- 定义路由规则
- 根路径重定向到主页面

### src/views/
页面级组件，包含业务逻辑和页面布局。
- 文件命名：`PascalCase.vue`（如 ExchangeRecord.vue）
- 使用 `<script setup lang="ts">` Composition API 风格

### src/components/
公共组件，可复用的 UI 组件。
- 文件命名：`PascalCase.vue`
- 使用 `<script setup lang="ts">` + `defineProps<T>` / `defineEmits<T>`

### src/api/
接口封装，封装与后端交互的接口。
- 按业务模块划分文件（如 exchange.ts）
- 统一管理接口路径和请求参数
- 支持 Mock/真实接口切换

### src/utils/
工具函数，提供通用功能。
- HTTP 请求封装（如 request.ts）

### src/mock/
Mock 数据，用于开发环境模拟接口。
- 提供模拟数据和模拟接口函数

### src/assets/
静态资源，如图片、字体等。

## 路径别名

`@/` 指向 `src/` 目录：

```typescript
import ExchangeRecord from '@/views/ExchangeRecord.vue'
import { getList } from '@/api/exchange'
import type { ExchangeRecord } from '@/mock'
```

## 相关文档

- 编码规范：`@docs/ai-context/coding-standards.md`
- 架构设计：`@docs/ai-context/architecture.md`
